...
Vector3 math::operator * (float scalar, const Vector3& vector)
{
...
std::ostream& math::operator<<(std::ostream& output, const Vector3& p)
{
	return output << "(" << p.x() << "," << p.y() << "," << p.z() << ")";
}

Vector3 math::saturate(const Vector3& color)
{
	Vector3 v;
...
Vector3 math::barycenter2d(int x0, int y0, int x1, int y1, int x2, int y2, int px, int py)
{
	int y0y2 = y0 - y2;
	int x1x2 = x1 - x2;
	int y1y2 = y1 - y2;
